...
func (a *Adapter) processPostAccountType(ctx *fiber.Ctx) error {
	...
	if e := json.Unmarshal(payload, &t); e != nil {
		...
		return ctx.JSON(pkg.RequestErrorWithRetry{
			Msg:         string(pkg.ErrorInvalidJSON),
			ShouldRetry: pkg.ShouldRetryRequest(400),
		})
	}
	...
	if e := v.Struct(t); e != nil {
		...
		_ = ctx.SendStatus(400)
		return ctx.JSON(pkg.RequestErrorWithRetry{
			Msg:         string(pkg.ErrorFailedRequestValidation),
			ShouldRetry: pkg.ShouldRetryRequest(400),
		})
	}
	...
	if e != nil {
		...
		return ctx.JSON(pkg.RequestErrorWithRetry{
			...
			ShouldRetry: pkg.ShouldRetryRequest(ctx.Response().StatusCode()),
		})
	}
...
func (a *Adapter) HandleCreateAccountType(in *string) (interface{}, error) {
	...
	if e != nil {
		...
		if strings.Contains(e.Error(), "duplicate key") {
			f = pkg.ErrorDuplicateKey
		} else {
			f = pkg.ErrorFailedAction
		}
...
func (a *Adapter) processDeleteAccountType(ctx *fiber.Ctx) error {
	...
	if e != nil {
		...
		return ctx.JSON(pkg.RequestErrorWithRetry{
			Msg:         e.Error(),
			ShouldRetry: pkg.ShouldRetryRequest(500),
		})
	}
...
func (a *Adapter) processPutAccountType(ctx *fiber.Ctx) error {
	...
	if e != nil {
		...
		return ctx.JSON(pkg.RequestErrorWithRetry{
			Msg:         string(pkg.ErrorInvalidJSON),
			ShouldRetry: pkg.ShouldRetryRequest(http.StatusBadRequest),
		})
	}
	...
	if e != nil {
		...
		return ctx.JSON(pkg.RequestErrorWithRetry{
			Msg:         e.Error(),
			ShouldRetry: pkg.ShouldRetryRequest(http.StatusInternalServerError),
		})
	}

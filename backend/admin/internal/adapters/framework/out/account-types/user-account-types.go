...
func (a *Adapter) InsertAccountType(data *[]byte) (*[]byte, error) {
  ...
  t := a.mongoActions.StorageAdapter.GetMongoAccountTypeActions().GetTimeStamp()
  ...
  if e != nil {
    ...
    if mongoError, ok := e.(mongo.WriteException); ok {
      a.mongoActions.WriteError.Msg = &mongoError.WriteErrors
  ...
  r := storage.InsertAccountOutput{InsertId: &id, TimeStamp: &t}
